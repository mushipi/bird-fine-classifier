...
- **監査の読み方（重要・2段前提）**: stage1(BirdNET)が「在群=科」判定したものだけ stage2 に届く。
  → 実際に来る OOD は **同科他種**（crow なら他のコルビッド）で、非同科(ヒヨドリ/スズメ等)は基本来ない。
  ∴ **主指標は『同科他種漏れ』**。非同科FP は高くても実害小（そもそも来ない入力）。
  ただし **stage1 の在群判定 precision が低いと非同科FPが効く**→ 配線時に stage1→stage2 ルーティング精度を一度確認。
  「保持≥0.90優先で FP は緩め」が正しいのは、効く側=同科他種漏れが別途低い前提（crow=0.115）。
- **同科他種漏れの受容度は『壁』の有無で決まる**: duck はカルガモ壁(音響的に割れない)で同科漏れ0.48を受容。
  crow は壁無しで0.115=ゲートが素直に効く。§5 混同で壁が出た群ほど同科漏れは受容側へ倒す（閾値で殴れない）。
